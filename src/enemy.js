...
class Enemy {
    ...
    trackPlayer(player, enemy) {
        ...
        if((Math.abs(distanceX) < 200 && Math.abs(distanceX) > 80) || (Math.abs(distanceY) < 200 && Math.abs(distanceY) > 80)){
            
            let velocityX = player.sprite.x - enemy.x;
            let velocityY = player.sprite.y - enemy.y;
